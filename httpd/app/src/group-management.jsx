...
/** @param {{groupId: string, active: 'feed' | 'settings' | 'members' | 'import'}} props */
export function GroupManagementNav({groupId, active}) {
  ...
  return <nav className="mb-6 flex gap-1 border-b border-border" aria-label="Feed management">
    ...
    {link('settings', `/groups/${groupId}/settings`, 'Settings')}
    {link('members', `/groups/${groupId}/members`, 'Members')}
    {link('import', `/feed/${groupId}/import`, 'Import Services')}
  </nav>;
}
