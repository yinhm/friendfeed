...
/** @param {{groupId: string, active: 'feed' | 'settings' | 'members' | 'import' | 'api', canManage?: boolean}} props */
export function GroupManagementNav({groupId, active, canManage = true}) {
  /** @param {'feed' | 'settings' | 'members' | 'import' | 'api'} name @param {string} href @param {string} label */
  ...
  return <nav className="mb-6 flex gap-1 border-b border-border" aria-label="Feed management">
    ...
    {canManage && link('api', `/feed/${groupId}/api`, 'API')}
  </nav>;
}
