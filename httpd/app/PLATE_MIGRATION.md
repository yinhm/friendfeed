...
- [ ] Replace removed `ELEMENT_*` and `MARK_*` constants with the corresponding plugin keys. Do not change the resulting strings: `p`, `h1`-`h6`, `blockquote`, `code_block`, `code_line`, `code_syntax`, `a`, `img`, `media_embed`, `ul`, `ol`, `li`, `action_item`, `emoji_input`, and the existing mark keys are persisted in `rawBody`. The v36 values are locked by `plate-plugin-keys.test.ts` before the v37 code migration begins.
