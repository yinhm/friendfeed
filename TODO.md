...
- [x] 增加一条 Playwright E2E 冒烟用例（编辑 → 提交 → 回显）：`scripts/e2e/` 起临时后端与 web（随机端口防僵尸冲突）、`ForceArchiveFeed` 播种，断言 `/public` 经 React + rawBody 路径渲染且无页面错误；`pnpm test:e2e` 驱动，两连跑验证可重复。
